		...
		<tr>
			<td>
				<form class="form-inline" id="filter-form" method="get" action="" onsubmit="return false;">
					<input class="form-control music-input text size1" type="text" name="q" value="{DATA_SEARCH.q}" placeholder="{LANG.video_name}"/>
					...
					<input class="music-button" type="button" value="{LANG.video_add}" onclick="window.location='{URL_ADD}';"/>
				</form>
			</td>
		</tr>
	</tbody>
	...
	<table class="table table-striped table-bordered table-hover">
		<thead>
			<tr>
				...
				<td class="center col-image-clip">{LANG.thumb}</td>
				<td><a href="{DATA_ORDER.title.data.url}" title="{DATA_ORDER.title.data.title}" class="{DATA_ORDER.title.data.class}">{LANG.video_name}</a></td>
				<td>{LANG.singer}</td>
				<td>{LANG.author}</td>
				<td>{LANG.filter_category}</td>
				<td><a href="{DATA_ORDER.numview.data.url}" title="{DATA_ORDER.numview.data.title}" class="{DATA_ORDER.numview.data.class}">{LANG.video_numview}</a></td>
		...
		<!-- BEGIN: row -->
			<tr class="topalign">
				...
				<td class="text-center"><a href="{ROW.thumb}" title="{ROW.title}" rel="shadowbox"><img src="{ROW.thumb}" alt="{ROW.title}" width="84" height="50"/></a></td>
				<td>{ROW.title}</td>
				<td>{ROW.singers}</td>
				<td>{ROW.authors}</td>
				<td>{ROW.theloai}</td>
				...
				<td class="text-center">
					<input name="status" id="change_status{ROW.id}" value="1" type="checkbox"{ROW.status} onclick="nv_change_videoclip_status({ROW.id})" />
				</td>
				<td class="text-center">
					<span class="edit-icon"><a class="nounderline" href="{ROW.url_edit}">{GLANG.edit}</a></span>
					<span class="delete-icon"><a class="nounderline" href="javascript:void(0);" onclick="nv_delete_videoclip({ROW.id});">{GLANG.delete}</a></span>
				</td>
			</tr>
		<!-- END: row -->
		<tbody>
		...
		<tfoot>
			<tr>
				<td colspan="10">
					<!-- BEGIN: action -->
					<span class="{ACTION.class}-icon"><a onclick="nv_videoclip_action(document.getElementById('levelnone'), '{LANG.alert_check}', {ACTION.key});" href="javascript:void(0);" class="nounderline">{ACTION.title}</a>&nbsp;</span>
					<!-- END: action -->
				</td>
			</tr>
		</tfoot>
