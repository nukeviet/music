	...
	<body>
		<div id="getuidcontent">
			...
			<table class="table table-striped table-bordered table-hover">
				<tbody>
					<tr><td colspan="8" class="center green"><strong>{LANG.getvideoid_title}</strong></td></tr>
					<tr>
		...
		<div id="resultdata">
			<table class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<td><a href="{DATA_ORDER.title.data.url}" title="{DATA_ORDER.title.data.title}" class="{DATA_ORDER.title.data.class}">{LANG.video}</a></td>
						<td>{LANG.singer}</td>
						<td>{LANG.author}</td>
			...
			function nv_close_pop( id, name ){
				...
				if( listvideo == "" ) listvideo = id;
				else listvideo = listvideo + "," + id;
				...
				$("input[name={RETURNINPUT}]", opener.document).val(listvideo);
				window.close()
			}
			</script>
		</div>
