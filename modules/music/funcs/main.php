...
{
	$sql = "SELECT b.id, b.name, b.tname, b.casi, b.thumb, b.listsong, c.ten AS singeralias, c.tenthat AS singername FROM " . NV_PREFIXLANG . "_" . $module_data . "_album_hot AS a INNER JOIN `" . NV_PREFIXLANG . "_" . $module_data . "_album` AS b ON a.albumid=b.id LEFT JOIN `" . NV_PREFIXLANG . "_" . $module_data . "_singer` AS c ON b.casi=c.id WHERE b.is_active=1 ORDER BY a.stt ASC";
}
else
{
	$sql = "SELECT a.id, a.name, a.tname, a.casi, a.thumb, a.listsong, b.ten AS singeralias, b.tenthat AS singername FROM `" . NV_PREFIXLANG . "_" . $module_data . "_album` AS a LEFT JOIN `" . NV_PREFIXLANG . "_" . $module_data . "_singer` AS b ON a.casi=b.id WHERE a.is_active=1 ORDER BY a.addtime DESC LIMIT 0,9";
}
...
$list = $nv_Cache->db( $sql, 0, $module_name );
