...
if( $nv_Request->isset_request( 'send_gift', 'post' ) )
{
	...
	$who_send = nv_substr( $nv_Request->get_title( 'who_send', 'post', '', 1 ), 0, 255);
	$who_receive = nv_substr( $nv_Request->get_title( 'who_receive', 'post', '', 1 ), 0, 255);
	$email_receive = nv_substr( $nv_Request->get_title( 'email_receive', 'post', '', 1 ), 0, 255);
	$body = nv_substr( $nv_Request->get_title( 'body', 'post', '', 1 ), 0, 500);
...
{
	$id = $nv_Request->get_int( 'id', 'post', 0 );
	$user_lyric = $nv_Request->get_title( 'user_lyric', 'post', '' );
	$body_lyric = $nv_Request->get_textarea( 'body_lyric', '', NV_ALLOWED_HTML_TAGS );
...
if( $nv_Request->isset_request( 'senderror', 'post' ) )
{
	...
	$user = $nv_Request->get_title( 'user', 'post', '' ); // Ten nguoi gui bao loi
	$body = $nv_Request->get_title( 'body', 'post', '', 1 );
	$where = $nv_Request->get_title( 'where', 'post', '' );
	$root_error = $nv_Request->get_title( 'root_error', 'post', '' );
...
if( $nv_Request->isset_request( 'votesong', 'post' ) )
{
	...
	else
	{
		...
		{
			$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . " SET binhchon = binhchon+1 WHERE id =" . $id );
			$nv_Request->set_Cookie( $module_name . '_vote_song_' . $id, NV_CURRENTTIME );
			$song = getsongbyID( $id );
			$numvote = $song['binhchon'];
			$endtime = NV_CURRENTTIME - 2592000;
			$result_time = $db->query( "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_topsong WHERE songid=" . $id . " LIMIT 1" );
			$nums_music = $result_time->rowCount();
			if( $nums_music == 0 )
			{
				$db->query( "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_topsong (id, songid, dt, hit) VALUES ( NULL, " . $db->quote( $id ) . ", " . $db->quote( NV_CURRENTTIME ) . ", 1 )" );
			}
			else
			{
				$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_topsong SET hit=hit+1 WHERE songid = " . $id );
			}
			$db->query( "DELETE FROM " . NV_PREFIXLANG . "_" . $module_data . "_topsong WHERE dt < " . $endtime . "" );
...
if( $nv_Request->isset_request( 'savealbum', 'post' ) )
{
	...
	$singer = $nv_Request->get_title( 'singer', 'post', '' );
	$message = nv_br2nl( $nv_Request->get_textarea( 'message', '', NV_ALLOWED_HTML_TAGS ) );
...
if( $nv_Request->isset_request( 'delsongfrplaylist', 'post' ) )
{
	...
	$db->query( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_playlist SET songdata = " . $db->quote( implode( ",", $songdatanew ) ) . " WHERE id =" . $plid );
	$nv_Cache->delMod( $module_name );
